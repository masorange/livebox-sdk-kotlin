...
/**
 * Converts a [DeviceSchedule] to a [WeekDayHour].
 * The ID on [DeviceSchedule] represents the hours in a week, so only values between 1 and 168 (24 x 7) are valid.
 *
 * @throws IllegalArgumentException if the DeviceSchedule ID is not between 1 and 168.
 */
@Throws(IllegalArgumentException::class)
fun DeviceSchedule.toWeekDayHour(): WeekDayHour {
    val id = id.toInt()
    if (id !in 1..168)
        throw IllegalArgumentException("DeviceSchedule ID must be between 1 and 168, inclusive.")
...
/**
 * Converts a [WeekDayHour] to a [DeviceSchedule].
 * The ID on [DeviceSchedule] is calculated as (dayIndex * 24 + hour + 1), where dayIndex is the index of the day in [WeekDay].
 *
 * @throws IllegalArgumentException if the WeekDayHour is invalid (day must be between 0 and 6, hour must be between 0 and 23).
 */
@Throws(IllegalArgumentException::class)
fun WeekDayHour.toDeviceSchedule(): DeviceSchedule {
    ...
    return DeviceSchedule(id = id.toString())
}